...
var (
	errInvalidSessionVariables = errors.New("invalid session variables")
)

// NewSessionVariables parse and create a session variables instance from string map
func NewSessionVariables(input map[string]string) (SessionVariables, error) {
	if len(input) == 0 {
		return nil, errInvalidSessionVariables
	}
	...
	if role, ok := result[types.XHasuraRole]; !ok || role == "" {
		return nil, errInvalidSessionVariables
	}
...
func NewSessionVariablesFromHeaders(header http.Header) (SessionVariables, error) {
	if len(header) == 0 {
		return nil, errInvalidSessionVariables
	}
...
func (sv SessionVariables) GetRole() string {
	return sv.Get(types.XHasuraRole)
}
...
func (sv SessionVariables) GetRequestID() string {
	return sv.Get(types.XRequestId)
}

// GetUserID gets the current user ID value
func (sv SessionVariables) GetUserID() string {
	return sv.Get(XHasuraUserID)
}

// IsAdmin checks if the current role is admin
func (sv SessionVariables) IsAdmin() bool {
	adminSecret := sv.Get(types.XHasuraAdminSecret)
	role := sv.GetRole()
	return adminSecret != "" && (role == "" || role == RoleAdmin)
}
